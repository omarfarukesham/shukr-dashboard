...
const DailyDuaPage = () => {
  ...
  // Data processing
  const duaOfTheDay = data?.data?.data?.duaOfTheDay || [];
  const totalPages = Math.ceil(duaOfTheDay.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const displayedPosts = duaOfTheDay.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => navigation("/daily-dua/add")}
          ...
          {displayedPosts.map((item, index) => (
            <TableRow key={item._id}>
              ...
              <TableCell className="flex gap-2 justify-center">
                <button
                  onClick={() => navigation(`/daily-dua/edit/${item._id}`)}
