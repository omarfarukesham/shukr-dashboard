...
const NewStickyPage = () => {
  ...
  // Data processing
  const whatNewItems = data?.data?.data?.whatNew || [];
  const totalPages = Math.ceil(whatNewItems.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const displayedPosts = whatNewItems.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => navigation("/sticky-notes/add")}
          ...
          {displayedPosts.map((item, index) => (
            <TableRow key={item._id}>
              ...
              <TableCell className="flex gap-2 justify-center">
                <button
                  onClick={() => navigation(`/sticky-notes/edit/${item._id}`)}
