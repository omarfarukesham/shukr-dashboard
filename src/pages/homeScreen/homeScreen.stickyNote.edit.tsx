...
const StickyNoteEditPage = () => {
  ...
  const onFormSubmit: SubmitHandler<Post> = async (data) => {
    ...
    try {
      await updateContentItem({ id: id as string, data: data }).unwrap();
      toast.success("Sticky Note Data has been updated...");
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-bold">Edit Sticky New Post</h1>
        <button
      ...
      <form onSubmit={handleSubmit(onFormSubmit)} className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md space-y-4">
        ...
        <div className="flex justify-center">
          <button
            type="submit"
            className="w-1/2 p-2 rounded-lg text-white bg-primary"
          >
            {isSubmiting ? "Processing..." : "Update"}
          </button>
        </div>
      </form>
    </div>
...
export default StickyNoteEditPage;
